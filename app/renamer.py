...
from app.rules import build_new_name, resolve_rules_for_file
...
def rename_files(folder_path, prefix="file", dry_run=True, 
    ...
    if recursive:
        ...
        _process_single_folder(root_path, prefix, dry_run, start_index, rules_config)
...
def _process_single_folder(folder_path, prefix, dry_run, start_index, rules_config):
    ...
    for filename in files:
        resolved_rule = resolve_rules_for_file(filename, rules_config)
        effective_prefix = resolved_rule.get("prefix", prefix)

        new_name = build_new_name(filename, current_index, effective_prefix)
...
def _process_files_global(all_files, prefix, dry_run, start_index, root_path, rules_config):
...
def _process_files_by_folder(all_files, prefix, dry_run, start_index, root_path, rules_config):
    ...
    current_index = start_index  # reinicio por carpeta
...
def _process_files_by_folder(all_files, prefix, dry_run, start_index, root_path):
    ...
    # 1. Agrupar por carpeta padre directa
    for file_path in all_files:
        files_by_folder[file_path.parent].append(file_path)

    # 2. Procesar cada carpeta de forma independiente
